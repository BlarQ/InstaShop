...
export default function Home() {
  ...
  <div className="flex items-center justify-center flex-col bg-white">
    ...
    <Image className="w-80 h-80 sm:w-60 sm:h-60" src={'/shoppingSale1.svg'} width={500} height={500} alt="Welcome Image" />
    ...
    <div className="bg-[#FFEAFA] p-4 border border-[#8A226F] w-full rounded-xl">
      {
        homeInfo.map((info) => (
          <div key={info.id} className="flex items-center justify-start py-1">
            <Image className="mr-4 size-6" src={info.image} width={40} height={40} alt={info.description} />
            <h2 className="text-[12px] sm:text-[14px] font-medium">{info.description}</h2>
          </div>
        ))
      }
    </div>
